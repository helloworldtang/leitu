...
public final class Caches {
    ...
    /** 进程内默认实现（TTL 惰性过期 + LRU 上限 + 同键只装载一次）；时钟取系统 UTC。 */
    ...
    public static <K, V> Cache<K, V> inMemory(long maxSize, Clock clock) {
        return new cn.youhuale.leitu.capability.cache.internal.InMemoryCache<>(
                maxSize, ExecutionContextReader.threadLocal(), clock);
    }
    ...
    public static <K, V> Cache<K, V> observing(Cache<K, V> delegate, ObservationRecorder recorder) {
        return new cn.youhuale.leitu.capability.cache.internal.ObservingCache<>(
                delegate, recorder, ExecutionContextReader.threadLocal());
    }
