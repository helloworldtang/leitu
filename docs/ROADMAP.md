...
1. **0.2.0 发版**——8 条答案 + CRUD 链闭合的完整故事，tag + GitHub Release（发版完备性检查全套走一遍；比 0.1.0 时多一项：能力宣称逐条对照交付物）。发完开发版复位 0.3.0-SNAPSHOT。
2. **starter 层规划（阶段三开启）**——第一个 Spring 模块，是整套架构决策（宿主版本管理 / BOM / 模块布局）：springdoc+knife4j 接线（api-contract drafted → answered）+ 各能力默认装配（三段交付的第三段「引依赖即用」）。
3. **dogfood**——真实项目引入一条链路，验证「可以拿去用」不是纸面判断（agent-platform 是既定候选）。
